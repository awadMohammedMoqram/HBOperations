...
public class AutoArchiveService(IServiceProvider serviceProvider, ILogger<AutoArchiveService> logger) : BackgroundService
{
    ...
    private async Task RunArchiveAsync(CancellationToken ct)
    {
        ...
        // Archive completed Mails older than cutoff
        var toArchive = await db.Mails
        ...
        {
            logger.LogDebug("No Mails to archive");
            return;
        }
        ...
        var docs = await db.MailAttachments
        ...
        logger.LogInformation("Auto-archived {Count} Mails ({Docs} documents), cutoff: {Days} days",
