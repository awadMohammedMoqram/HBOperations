...
internal static class ExcelHelpers
{
    ...
    public static void AddSummarySheet(IXLWorkbook workbook, string title, ReportSummary summary, string generatedBy, DateTime reportDate)
    {
        ...
        WriteSummaryRow(ws, 5, "إجمالي البريد", summary.Total, BankPrimary);
    ...
    public static void AddAuditTrailSheet(IXLWorkbook workbook, List<AuditTrailReportRow> rows, string sheetName = "سجل المراجعة")
    {
        ...
        foreach (var r in rows)
        {
            ...
            {
                MailStatus.Received or MailStatus.Archived => LightGreen,
                MailStatus.Rejected => LightRed,
                MailStatus.Sent or MailStatus.AssignedToStaff => LightOrange,
                _ => XLColor.NoColor
            };
            row++;
        }
    ...
    {
        var ws = workbook.Worksheets.Add("بريدي");
        ws.RightToLeft = true;
    ...
    private static string GetStatusName(MailStatus s) => s switch
    {
        MailStatus.Sent => "مُرسلة",
        MailStatus.AssignedToStaff => "موجه لموظف",
        MailStatus.Received => "مستلمة",
