...
public class PdfReportService
{
    ...
    public byte[] GenerateBranchReport(List<BranchReportRow> data, ReportSummary summary, string generatedBy, DateTime reportDate)
    {
        ...
        {
            ComposeSummaryBox(col, "يعرض هذا التقرير تحليلاً شاملاً لحركة البريد الواردة والصادرة عبر جميع فروع البنك، مع توضيح مؤشرات الأداء الرئيسية وحالات البريد.");
            ...
            if (data.Count > 0)
            {
                ComposeSection(col, "النتائج والملاحظات", section =>
                {
                    ...
                    Observation(section, "info", $"أعلى فرع نشاطاً: {topBranch.BranchName} بإجمالي {topBranch.Total} بريد");
                    ...
                    if (summary.AvgProcessingHours > 0)
                        Observation(section, "warn", $"متوسط زمن البريد: {summary.AvgProcessingHours:F1} ساعة");
                });
            }
        });
    }
    ...
    public byte[] GenerateDepartmentReport(List<DepartmentReportRow> data, ReportSummary summary, string generatedBy, DateTime reportDate)
    {
        ...
        {
            ComposeSummaryBox(col, "يستعرض هذا التقرير أداء الإدارات المختلفة في معالجة البريد الواردة والصادرة، مع تحديد الإدارات الأكثر نشاطاً والأكثر كفاءة في الإنجاز.");
            ...
            if (data.Count > 0)
            {
                ComposeSection(col, "النتائج والملاحظات", section =>
                {
                    ...
                    Observation(section, "info", $"أعلى إدارة نشاطاً: {topDept.DepartmentName} بإجمالي {topDept.Total} بريد");
    ...
    public byte[] GeneratePerformanceReport(List<PerformanceReportRow> data, ReportSummary summary, string generatedBy, DateTime reportDate, string scopeLabel)
    {
        ...
        {
            ComposeSummaryBox(col, "يقدم هذا التقرير تحليلاً مفصلاً لمؤشرات الأداء الرئيسية شاملاً عدد البريد المعالجة، نسبة الإنجاز، ومتوسط زمن المعالجة لكل جهة.");
            ...
            ComposeSection(col, "مؤشرات الأداء التفصيلية", section =>
            {
                section.Item().Element(container =>
                {
                    container.Border(1).BorderColor(GrayLight).Table(table =>
                    {
                        ...
                        TableHeader(table, ["#", "الاسم", "البريد", "مكتملة", "مرفوضة", "متوسط (ساعة)", "نسبة الإنجاز"]);
    ...
    public byte[] GenerateAdminAffairsReport(List<AdminAffairsReportRow> data, ReportSummary summary, string generatedBy, DateTime reportDate)
    {
        ...
        {
            ComposeSummaryBox(col, "يوضح هذا التقرير أداء فريق الشؤون الإدارية في استلام وتسليم البريد بين الفروع والإدارات، مع تحليل زمن التوصيل والبريد المعلّقة.");
            ...
            if (data.Count > 0)
            {
                ComposeSection(col, "النتائج والملاحظات", section =>
                {
                    ...
                    Observation(section, totalPending > 5 ? "warn" : "info", $"إجمالي البريد المعلّقة: {totalPending}");
                });
            }
        });
    }
        ...
        return Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Content().PaddingHorizontal(24).PaddingTop(18).PaddingBottom(10).Column(col =>
                {
                    ...
                    col.Item().Row(row =>
                    {
                        StatCard(row, summary.Total.ToString(), "إجمالي البريد", Primary);
                        row.ConstantItem(10);
                    ...
                    col.Item().PaddingTop(12).Row(row =>
                    {
                        ...
                        row.RelativeItem().Background(White).Border(1).BorderColor(GrayLight)
                            .Padding(14).Row(r =>
                            {
                                r.RelativeItem().AlignMiddle().Text("متوسط وقت البريد").FontSize(10).Bold().FontColor(Gray);
    ...
    public byte[] GenerateAuditTrailReport(List<AuditTrailReportRow> rows, ReportSummary summary, string generatedBy, DateTime reportDate)
    {
        return GenerateDocument("تقرير المراجعة والتدقيق", summary, generatedBy, reportDate, col =>
        {
            ComposeSummaryBox(col,
                "يعرض هذا التقرير المخصّص للمدققين والمراجعين السجل الكامل للبريد بما فيها الحقول الحساسة (ملاحظات الرفض والإدارة) لأغراض التدقيق والمتابعة.");

            ComposeSection(col, $"سجل البريد ({rows.Count} بريد)", section =>
            {
                section.Item().Element(container =>
                {
                    container.Border(1).BorderColor(GrayLight).Table(table =>
            ...
            if (rows.Count > 0)
            {
                ComposeSection(col, "ملخص التدقيق", section =>
                {
                    ...
                    Observation(section, rejected > 0 ? "warn" : "info", $"عدد البريد المرفوضة في الفترة: {rejected}");
                    Observation(section, "info", $"بريد بملاحظات إدارية: {withAdminNote}");
    ...
    // ═══════════════════════════════════════════════════════════════
    //  Personal Report (للموظف الفردي — بريده فقط)
    // ═══════════════════════════════════════════════════════════════
    ...
    public byte[] GeneratePersonalReport(List<PersonalReportRow> rows, ReportSummary summary, string generatedBy, DateTime reportDate)
    {
        ...
        {
            ComposeSummaryBox(col,
                "ملخص لبريدك الشخصية في الفترة المحددة، يشمل البريد الصادرة منك والواردة إليك مع حالتها الحالية.");

            ComposeSection(col, "قائمة بريدي", section =>
            {
                section.Item().Element(container =>
                {
                    container.Border(1).BorderColor(GrayLight).Table(table =>
    ...
    public byte[] GenerateShariahReport(List<AuditTrailReportRow> cashRows, ReportSummary summary, string generatedBy, DateTime reportDate)
    {
        ...
        {
            ComposeSummaryBox(col,
                "تقرير مخصّص للمراجع الشرعي يستعرض البريد المالية (التحويلات النقدية) للتحقق من مطابقتها لأحكام الشريعة الإسلامية ومعايير الرقابة الشرعية في البنك.");

            ComposeSection(col, $"البريد النقدية ({cashRows.Count} بريد)", section =>
            {
                section.Item().Element(container =>
                {
                    container.Border(1).BorderColor(GrayLight).Table(table =>
            ...
            ComposeSection(col, "ملاحظات شرعية", section =>
            {
                ...
                Observation(section, summary.Rejected > 0 ? "warn" : "success",
                    $"بريد مرفوضة (تستوجب المراجعة الشرعية): {summary.Rejected}");
                Observation(section, "info",
                    $"بريد مكتملة بنجاح: {summary.Completed} ({summary.CompletionRate:F1}%)");
        ...
        DateTime reportDate)
    {
        ...
        {
            ComposeSummaryBox(col,
                "يُحلل هذا التقرير البريد المرفوضة في الفترة المحددة لتحديد الأنماط المتكررة والجهات الأكثر إصداراً للرفض، بهدف تحسين جودة البريد وتقليل نسب الرفض.");
            ...
            // التفاصيل
            ComposeSection(col, $"تفاصيل البريد المرفوضة ({rejected.Count})", section =>
            {
                section.Item().Element(container =>
                {
                    container.Border(1).BorderColor(GrayLight).Table(table =>
            ...
            ComposeSection(col, "توصيات", section =>
            {
                if (byType.Count > 0)
                {
                    ...
                    Observation(section, "warn", $"النوع الأكثر رفضاً: {topType.Label} ({topType.Count} بريد، {topType.Percentage:F0}%)");
                }
                if (byBranch.Count > 0)
                {
                    var topBranch = byBranch.OrderByDescending(g => g.Count).First();
                    Observation(section, "warn", $"الفرع الأكثر رفضاً: {topBranch.Label} ({topBranch.Count} بريد)");
                }
                Observation(section, "info", "يُنصح بمراجعة معايير قبول البريد وتدريب الموظفين على الأنواع الأكثر رفضاً.");
            });
        });
    }
    ...
    public byte[] GenerateExecutiveReport(ExecutiveReportData data, string generatedBy, DateTime reportDate)
    {
        ...
        {
            ComposeSummaryBox(col,
                $"تقرير تنفيذي شامل للإدارة العليا يستعرض الأداء الإجمالي للبنك خلال الفترة ({data.PeriodLabel})، يشمل أداء الفروع والإدارات وتوزيع أنواع البريد والاتجاهات الزمنية.");
            ...
            {
                ComposeSection(col, "توزيع البريد حسب النوع", section =>
                {
            ...
            {
                Observation(section, "info", $"إجمالي البريد في الفترة: {data.Summary.Total}");
                ...
                if (data.TotalRejected > 0)
                    Observation(section, "warn", $"عدد البريد المرفوضة: {data.TotalRejected} ({(data.Summary.Total > 0 ? (double)data.TotalRejected / data.Summary.Total * 100 : 0):F1}%)");
                if (data.Branches.Count > 0)
                {
                    var top = data.Branches.OrderByDescending(b => b.Total).First();
                    Observation(section, "success", $"أعلى فرع نشاطاً: {top.BranchName} ({top.Total} بريد)");
                }
            });
        });
    }
    ...
    private static string GetTypeName(MailType t) => t switch
    {
        MailType.DocumentDelivery => "تسليم مرفقات",
        MailType.CashTransfer => "تحويل نقدي",
        MailType.InternalDepartment => "داخلي",
        MailType.BranchToBranch => "بين الفروع",
        MailType.Other => "أخرى",
        _ => t.ToString()
    };

    private static void StatusBadgeCell(TableDescriptor table, MailStatus status, string rowBg)
    {
        var (color, bgColor, label) = status switch
        {
            MailStatus.Received or MailStatus.Archived => (Success, SuccessBg, "مستلمة"),
            MailStatus.Sent or MailStatus.AssignedToStaff => (Warn, WarnBg, "قيد التسليم"),
            MailStatus.Rejected => (Danger, DangerBg, "مرفوضة"),
